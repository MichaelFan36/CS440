    ...
    for i in range(len(dev_set)):
        ...
        for word in dev_set[i]:
            if word in word_counter:
                score = word_dict[(i, word)] / len(dev_set[i]) * np.log( len(train_set) / (1 + word_counter[word]))
            else:
                score = word_dict[(i, word)] / len(dev_set[i]) * np.log(len(train_set))
            if score > tf_idf:
                tf_idf = score
                goal_word = word
        result.append(goal_word)
