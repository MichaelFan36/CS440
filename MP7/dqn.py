...
def make_dqn(statesize, actionsize):
    ...
    """
    model = nn.Sequential(nn.Linear(statesize, 20), nn.ReLU(), nn.Linear(20, actionsize), nn.ReLU())
    return model
...
class DQNPolicy(QPolicy):
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        """
        target = reward + self.gamma * torch.max(torch.from_numpy(self.qvals(next_state))) * (1-done)
        self.model.train()
        states = torch.from_numpy(state).type(torch.FloatTensor)
